...
def test_unknown_server_held_in_pending(tmp_path: Path) -> None:
    ...
    admitted, admitted_cache = agent._apply_consent_gate(manifests, {})

    assert admitted == []
    assert admitted_cache == {}
...
def test_approved_server_admitted_through_gate(tmp_path: Path) -> None:
    ...
    agent = _agent_with_home(tmp_path)
    admitted, _ = agent._apply_consent_gate(manifests, {})
...
def test_approved_server_with_changed_catalog_held_pending(tmp_path: Path) -> None:
    ...
    agent = _agent_with_home(tmp_path)
    admitted, _ = agent._apply_consent_gate(expanded, {})
...
def test_gate_per_server_isolation(tmp_path: Path) -> None:
    ...
    agent = _agent_with_home(tmp_path)
    admitted, _ = agent._apply_consent_gate(github_skills + acme_skills, {})
...
def test_approve_persists_for_future_sessions(tmp_path: Path) -> None:
    ...
    admitted, _ = agent_2._apply_consent_gate(manifests, {})
    assert {m.name for m in admitted} == {"alpha"}
...
def test_archive_cache_scoping_does_not_collide_on_prefix(tmp_path: Path) -> None:
    """`_apply_consent_gate` must not attribute a cache entry for
    `skill://foo` to a server publishing `skill://foobar` — and vice
    versa. Bare `startswith` on the root URI without a `/` boundary
    collides on neighboring roots."""
    foo_manifest = SkillManifest(
        name="foo",
        description="foo",
        body="",
        path=None,
    ...
    foobar_manifest = SkillManifest(
        name="foobar",
        description="foobar",
        body="",
        path=None,
        uri="skill://foobar/SKILL.md",
        server_name="srv-foobar",
    )
    cache = {
        "skill://foo": {"SKILL.md": b"foo body"},
        "skill://foobar": {"SKILL.md": b"foobar body"},
    }
    agent = _agent_with_home(tmp_path)
    agent._apply_consent_gate([foo_manifest, foobar_manifest], cache)

    # Each pending server only sees its own cache.
    assert set(agent._pending_mcp_archive_cache["srv-foo"].keys()) == {"skill://foo"}
    assert set(agent._pending_mcp_archive_cache["srv-foobar"].keys()) == {
        "skill://foobar"
    }
