...
async def test_archive_cache_read_is_wrapped() -> None:
    """Cache-served reads come from an MCP-published archive; the
    wrapper fires identically to the live aggregator path so the
    transcript marker doesn't change based on whether the read was
    cached."""
    ...
    assert "</mcp-skill-content>" in text
